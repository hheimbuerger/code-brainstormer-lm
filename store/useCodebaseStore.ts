  ...
  devtools(
    temporal(
      (set, get) => ({
        ...
        // Derived flags
        get canUndo() {
          const t = (get() as any).temporal;
          const s = t?.getState?.();
          return (s?.pastStates?.length ?? 0) > 0;
        },
        get canRedo() {
          const t = (get() as any).temporal;
          const s = t?.getState?.();
          return (s?.futureStates?.length ?? 0) > 0;
        },
        ...
        addCodeFunction: (position: { x: number; y: number }) => {
          set((state) => {
            ...
            return { codeFunctions, nodePositions };
          }, false, 'addCodeFunction');
        },
        
        // Set position for a specific node
        setNodePosition: (index: number, position: { x: number; y: number }) => {
          // Called on drag end per ProjectCanvas; results in one history entry via zundo
          set((state) => ({
            nodePositions: {
              ...state.nodePositions,
              [index]: position,
        ...
        updateCodeFunction: (index: number, func: Partial<CodeFunction>) => {
          set((state) => {
            ...
            return { codeFunctions } as Partial<CodebaseState>;
      ...
      {
        // Track only the domain we want under history
        partialize: (state) => ({
          projectName: state.projectName,
          codeFunctions: state.codeFunctions,
