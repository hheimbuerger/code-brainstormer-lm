...
const EditableField = ({
  ...
  const handleSave = () => {
    // Only save and trigger aspect state change if the value actually changed
    if (inputValue !== value) {
      onSave(inputValue);
    }
    setIsEditing(false);
...
function FunctionNode(props: FunctionNodeProps) {
    ...
    while ((match = regex.exec(impl)) !== null) {
      ...
      if (targetFunctionExists) {
        ...
        parts.push(
          ...
          >
            {fnCall}
            <div className="new-function-indicator">
              ⭐
            </div>
          </span>
        );
  ...
  const handleNodeClick = (e: React.MouseEvent) => {
    ...
    // Prevent node selection when clicking on non-identifier areas
    if (!target.closest('.method-node__identifier')) {
      e.stopPropagation();
    }
  };
