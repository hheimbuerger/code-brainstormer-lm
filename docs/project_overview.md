...
| **Docs** | `docs/` | Architectural notes (`architecture_data_model.md`), comprehensive user interaction guide (`user_interactions.md`), and this overview. |
...
9. **Intelligent Node Creation**: Three distinct double-click behaviors for intuitive function creation and navigation:
   - **Canvas double-click**: Creates empty function with auto-focus for immediate typing
   - **Unresolved function reference double-click**: Creates named function from orange function calls
   - **Existing function reference double-click**: Smooth navigation to existing functions
10. **Smart Node Placement**: Distance-based placement algorithm with 20px grid alignment, 50px margins, and collision detection for optimal layouts.
11. **Smooth Viewport Animation**: 800ms animations for all navigation with zoom preservation and accurate centering.
12. **Visual Function Call Indicators**: Blue styling for existing functions, orange with asterisk for unresolved functions, with hover effects and tooltips.
13. **Selective Auto-Focus**: Context-aware auto-focus that only triggers for empty nodes, preserving workflow for pre-filled identifiers.
